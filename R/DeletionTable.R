...
DeletionTable <-
  function(Directory,
    ...
    while (i < mx) {
      ...
      command <-
        paste("/home/zhangs3/Data/Anaconda3-envs/aligners/bin/samtools depth -r",
              loc,
              paste0('../',
                     temp_dir,
                     '/',
              ...
              '> reads-per-position.txt',
              sep = " ")
      system(command)
      system("awk -F \" \" '($3 > 5) {print $0}' reads-per-position.txt > reads-per-position2.txt")
      chr <-
        read.delim("reads-per-position2.txt",
                   header = F)
      colnames(chr) <-
        c("chr", "start", "Depth")
      x2 <-
